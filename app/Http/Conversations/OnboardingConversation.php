...
class OnboardingConversation extends Conversation
{
    ...
    {
        $question = Question::create('Which kind of Pizza do you want ?')
                ->callbackId('select_service')
                ->addButtons([
    ...
    public function askDetails(){
        $this->say('Got it. Let me just take your info to place your order 😊');
            ...
            $this->ask('Finally the delivery address ?', function (Answer $answer) {
                ...
                $this->ask($question, function (Answer $answer) {
                    switch($answer->getValue()){
                        ...
                        case 3 : 
                            $this->bot->userStorage()->save([
                                'delivery_time' => now()->addHour($answer->getValue())->format('H:i:s')
                            ]);
    ...
    public function trackOrder(){
        ...
        $this->ask('Enter your Order ID to track your order ?', function (Answer $answer) {
            $this->bot->userStorage()->save([
                'OrderID' => $answer->getText(),
            ]);
            $user = $this->bot->userStorage()->find();
            $order = Order::find($user->get('OrderID'));
            if(empty($order)){
                $this->say('Order Not Found With this ID');
                $this->askBuy();
            }else{
                $this->say('Hi'.$order->user_name.', You will get your pizza at "'. $order->delivery_time.'" <br><br>');
            }
        });
    }
